...
#define component(w, c, i) w->getComponent<c>(i)
        ...
        wInve = new Window(dict("gui.window.inventory"), sf::Vector2f(800, 400), sf::Vector2f(rw.getSize().x/2-400, rw.getSize().y/2-200));
            for (int i=0, j=0; i<8 && j<4; i++, j++)
                wInve->addComponent(new Slot(sf::Vector2f(90, 90), sf::Vector2f(5+i*95, 20+j*95)), true);
