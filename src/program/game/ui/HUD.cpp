...
namespace rr
{
    ...
    HUD::update(Player* p, int lvl, sf::Time& time)
    {
        ...
        {
            if      (p->getBuffs().hunger >= 500 && p->getBuffs().hunger < 1000) m_bHP.setFillColor(sf::Color(0xff, 0x40, 0x40));
            else if (p->getBuffs().hunger >= 1000)                               m_bHP.setFillColor(sf::Color(0xff, 0xaa, 0xaa));
            else                                                                 m_bHP.setFillColor(sf::Color::Red);
        }
