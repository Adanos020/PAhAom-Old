...
namespace rr {
    ...
    void Menu::addOption(sf::String option) {
        options_.push_back(Button(position_ + sf::Vector2f(0, options_.size()*40), option, 25));
    }

    void Menu::setOption(unsigned index, sf::String option) {
        options_[index].getText().setString(option);
    }

    void Menu::removeOption(unsigned index) {
    ...
    sf::String Menu::getChosenOption(sf::RenderWindow& rw, sf::Event& event) {
