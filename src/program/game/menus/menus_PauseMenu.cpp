...
namespace rr {
    ...
    PauseMenu::PauseMenu(sf::RenderWindow& rw) {
        ...
        if (settings.language=="fc")
            ...
            wMenu->addComponent(new Button(wOpts, sf::Vector2f(0, 5), dict("gui.button.resume"), 52));
            wMenu->addComponent(new Button(wOpts, sf::Vector2f(0, 80), dict("gui.button.options"), 52));
            wMenu->addComponent(new Button(wOpts, sf::Vector2f(0, 155), dict("gui.button.help"), 52));
            wMenu->addComponent(new Button(wOpts, sf::Vector2f(0, 230), dict("gui.button.quit"), 52));
            for (int i=0; i<4; i++)
        ...
        wOpts = new Window(nullptr, dict("gui.button.options"), sf::Vector2f(330, 454), sf::Vector2f(rw.getSize().x-355, rw.getSize().y/2-220));
